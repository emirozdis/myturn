      ...
      <AnimatePresence>
        {!isPlayingCompilation && (
          <motion.div
            ...
            className="fixed inset-0 bg-[#0a0a0a] overflow-y-auto overflow-x-hidden flex flex-col select-none pointer-events-auto"
          >
            {/* Particles rendered on global background layer behind all layout contents */}
            <ParticleEffect type="confetti" active={showConfetti} />

            {/* Close Button padded properly with env safe area and glass styles checks */}
            <button
              onClick={onClose}
              className="absolute right-6 w-10 h-10 rounded-full flex items-center justify-center text-white/60 hover:text-white hover:bg-white/10 transition-colors z-20 pointer-events-auto"
              style={{ 
            ...
            <div className="flex-1 flex flex-col px-6 pt-16 pb-8 relative z-10 w-full max-w-md mx-auto min-h-full">
              ...
              {/* Media Card */}
              <div className="relative w-full aspect-square sm:aspect-[4/3] rounded-[24px] bg-neutral-900 border border-white/10 overflow-hidden mb-5 shadow-lg group cursor-pointer" onClick={() => playCompilation("full")}>
                <img
