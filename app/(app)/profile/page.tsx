        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "8px 4px", textAlign: "center" as const }}>
          ...
          {CALENDAR_DAYS.map((item, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "center", position: "relative" }}>
              ...
              {item.best && (
                <div style={{ position: "absolute", bottom: -2, right: -2, width: 13, height: 13, background: "#fff", borderRadius: "50%", display: "flex", alignItems: "center" as const, justifyContent: "center" as const, border: `2px solid #111` }}>
    ...
    <div style={{ display: "flex", flexDirection: "column" as const, gap: 10 }}>
      {USER_GROUPS.map(g => (
        <div key={g.id} style={{ display: "flex", alignItems: "center", gap: 12, ...glassStyle(0.04, 20, 0.08), borderRadius: 14, padding: "12px 14px" }}>
          <div style={{ width: 46, height: 46, borderRadius: 12, background: "rgba(255,255,255,0.07)", display: "flex", alignItems: "center" as const, justifyContent: "center" as const, fontSize: 22, flexShrink: 0 }}>
            {g.emoji}
          </div>
      ...
      {SAVED_VLOGS.map(v => (
        <div key={v.id} style={{ display: "flex", gap: 12, ...glassStyle(0.04, 20, 0.08), borderRadius: 14, overflow: "hidden", cursor: "pointer" }}>
          <div style={{ position: "relative", width: 90, flexShrink: 0 }}>
            <img src={v.thumb} alt={v.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
            <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center" as const, justifyContent: "center" as const }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center" as const, justifyContent: "center" as const }}>
          ...
          <div style={{ flex: 1, padding: "12px 12px 12px 0", display: "flex", flexDirection: "column" as const, justifyContent: "space-between" }}>
            <div>
...
export default function ProfilePage() {
  ...
  const settingsRows = [
    ...
    { icon: Lock,       label: "Privacy",         sub: "Manage your privacy settings", panel: "privacy" as const },
    { icon: Bell,       label: "Notifications",   sub: "Manage your notifications",    panel: "notifications" as const },
    { icon: Palette,    label: "Theme",           sub: "Dark Mode",                    panel: "theme" as const, right: "moon" },
  ];
